...
function parseArgs(): RequeueOptions {
...
function shouldRequeueEntry(entry: DLQEntry, options: RequeueOptions): boolean {
...
async function requeueDLQEntries() {
  ...
  try {
    ...
    const entriesToRequeue: DLQEntry[] = [];
    const skippedEntries: DLQEntry[] = [];
    ...
    for (const entry of entriesToRequeue) {
      try {
        ...
        const messageId = await xaddObj(redis as any, entry.streamKey, originalFields, {
